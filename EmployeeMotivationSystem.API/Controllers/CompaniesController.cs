...
{
    public CompaniesController(AppDbContext dbContext) : base(dbContext) { }
    ...
    {
        var company =
            await DbContext.Companies.SingleOrDefaultAsync(el => el.Id == id);
    ...
    public async Task<GetCompanyMembersByIdResponseApiDto>
    GetCompanyMembersById(int id)
    {
        var company =
            await DbContext.Companies.SingleOrDefaultAsync(el => el.Id == id);
        ...
        var members = await DbContext.CompaniesUsers.Include(el => el.Company)
                          .Include(el => el.User)
    ...
    public async Task<GetCompanyMetricsByIdResponseApiDto>
    GetCompanyMetricsById([FromBody] GetCompanyMetricsByIdRequestApiDto request)
    {
        var member = await DbContext.CompaniesUsers.SingleOrDefaultAsync(
            el => el.UserId == request.UserId);

        if (member == null)
            throw new Exception(
                $"Member with id: {request.UserId} not found or not pinned to any company!");

        var metricIdToMetricCount =
            await DbContext.CompaniesUsersMetrics
                ...
                .Join(DbContext.Metrics, obj1 => obj1.Key, obj2 => obj2.Id,
                      (obj1, obj2) =>
                          new
                          {
                              Id = obj2.Id,
        ...
        var metrics = metricIdToMetricCount.Select(
            el => new Metric
            {
                Id = el.Id,
        ...
        {
            Items = metricIdToMetricCount.Select(
              el => new GetCompanyMetricsByIdResponseApiDto
                        .GetCompanyMetricsByIdItemResponseApiDto
              {
                  ...
                  TargetValue = el.TargetValue,
                  Count = el.Count,
                  Bonus = UsersController.CalculateBonuses(
                              member.Salary, metrics, el.Count)
              })
        };
    }

    [HttpPost("AddMetricToMember")]
    public async Task<AddMetricToMemberResponseApiDto>
    AddMetricToMember([FromBody] AddMetricToMemberRequestApiDto request)
    {
        await DbContext.CompaniesUsersMetrics.AddRangeAsync(request.Items.Select(
            el => new CompaniesUsersMetric
            {
    ...
    public async Task<GetCompanyPositionByIdResponseApiDto>
    GetCompanyPositionsById(int id)
    {
        if (HttpContext.Items[JwtMiddleware.JwtTokenHttpContextKey] is not string
                userEmail)
            throw new Exception(
                "Very strange. Your email is null. I think something its broken.");

        var userFromJwt =
            await DbContext.Users.SingleOrDefaultAsync(el => el.Email == userEmail);

        if (userFromJwt == null)
            throw new Exception("User from jwt is bad, its not good.");

        var userFromJwtCompanies =
            await DbContext.CompaniesUsers
        ...
        var positions =
            await DbContext.Positions.Where(el => el.CompanyId == company.Id)
    ...
    [HttpPost]
    public async Task<CreateCompanyResponseApiDto>
    CreateCompany([FromBody] CreateCompanyRequestApiDto request)
    {
        if (HttpContext.Items[JwtMiddleware.JwtTokenHttpContextKey] is not string
                userEmail)
            throw new Exception(
        ...
        if (user == null)
            throw new Exception(
                "Very strange. You is null. I think something its broken.");

        var userCompany = await DbContext.Companies.SingleOrDefaultAsync(
            el => el.CreatorUserId == user.Id);

        if (userCompany != null)
            throw new Exception(
                $"User already has company with name: {userCompany.Name}");

        var newCompany = await DbContext.Companies.AddAsync(
            new Company { Name = request.Name, CreatorUserId = user.Id });

        await DbContext.SaveChangesAsync();

        var newPosition = await DbContext.Positions.AddAsync(
            new Position
            {
        ...
        await DbContext.CompaniesUsers.AddAsync(
            new CompaniesUser
            {
                CompanyId = newCompany.Entity.Id,
                UserId = user.Id,
    ...
    UpdateCompanyMember([FromForm] UpdateCompanyMemberRequestApiDto request)
    {
        var companiesUser = await DbContext.CompaniesUsers.SingleOrDefaultAsync(
            el => el.CompanyId == request.CompanyId &&
                  el.PositionId == request.PositionId &&
                  el.UserId == request.MemberId);

        if (companiesUser == null)
            throw new Exception($"" + $"CompanyUser by " +
                                $"companyId: {request.CompanyId}, " +
                                $"positionId: {request.PositionId} " +
                                $"memberId: {request.MemberId}" + $"not found!");
    ...
    public async Task DeleteCompanyMember(int companyId, int memberId)
    {
        ...
        var user =
            await DbContext.Users.SingleOrDefaultAsync(el => el.Id == memberId);
